...
콘텍스트 스위치(Context Switch)가 일어날 때 실행되고 있는 프로세스의 PCB에 현재의 주소를 저장하고, 다음 실행할 프로세스의 PCB에서 프로그램 카운터를 불러온다.
...
Synchronization(동기화) 문제 발생

### 메세지 패싱(Message Passing)

send와 receive를 통한 메세지 교환
 ...
 * receive(id, message) - 아무곳에서나 메세지를 받음
  * id가 상황마다 communication을 필요로 하는 프로세스의 이름으로 정해짐
